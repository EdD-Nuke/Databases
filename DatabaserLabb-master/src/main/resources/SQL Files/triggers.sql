...
CREATE FUNCTION unregisterFunction() RETURNS TRIGGER AS $$
...
BEGIN
    ...
    IF (EXISTS(SELECT student, course FROM Registrations WHERE (student, course)=(OLD.student, OLD.course))) THEN
        IF student = 0 THEN
...
FOR EACH ROW EXECUTE FUNCTION unregisterFunction();
